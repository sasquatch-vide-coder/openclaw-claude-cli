...
# OpenClaw Custom Installer — installs from sasquatch-vide-coder/openclaw-claude-cli fork
# Usage: bash install-custom.sh
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 OpenClaw Custom Installer")"
        tagline="$("$GUM" style --foreground "#8892b0" "$TAGLINE")"
        hint="$("$GUM" style --foreground "#5a6480" "installing from sasquatch-vide-coder/openclaw-claude-cli")"
    ...
    echo -e "${NC}${INFO}  Installing from sasquatch-vide-coder/openclaw-claude-cli${NC}"
    echo ""
}
...
GIT_DIR_DEFAULT="${HOME}/openclaw-claude-cli"
...
install_openclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/sasquatch-vide-coder/openclaw-claude-cli.git"
